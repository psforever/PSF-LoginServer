...
object WorldConfig extends ConfigParser {
  ...
  protected val hostname_pattern = Constraints.pattern(
    raw"^((([a-zA-Z0-9]|[a-zA-Z0-9][a-zA-Z0-9\-]*[a-zA-Z0-9])\.)*([A-Za-z0-9]|[A-Za-z0-9][A-Za-z0-9\-]*[A-Za-z0-9]))|()$$".r,
    "hostname"
  )
  ...
  protected val configTemplate = Seq(
    ...
    ConfigSection("psadmin", ConfigEntryInt("ListeningPort", 51002, Constraints.min(1), Constraints.max(65535))),
    ConfigSection(
      "developer",
    ...
    ConfigSection("kamon", ConfigEntryBool("Active", false)),
    ConfigSection(
      "antihack",
