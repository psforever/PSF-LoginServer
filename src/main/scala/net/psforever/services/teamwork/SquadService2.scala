...
import net.psforever.packet.game.{PlanetSideZoneID, SquadDetail, SquadPositionDetail, SquadPositionEntry, SquadAction => SquadRequestAction, SquadInfo}
import net.psforever.services.Service
import net.psforever.types.{PlanetSideEmpire,PlanetSideGUID,  SquadRequestType, SquadResponseType}

class SquadService2 extends Actor {
  import SquadService2._
  ...
  /**
    ...
    */
  private val searchData: mutable.LongMap[SquadService2.SearchCriteria] =
    mutable.LongMap[SquadService2.SearchCriteria]()
  ...
  private val invitations = context.actorOf(Props(classOf[SquadInvitationManager], subs), name="invitations-manager")
  ...
  override def postStop(): Unit = {
    ...
    searchData.clear()
    subs.postStop()
    invitations.tell(akka.actor.PoisonPill, self)
  }
  ...
  def receive: Receive = {
    ...
    case SquadInvitationManager.PerformStartSquad(invitingPlayer) =>
      performStartSquad(sender(), invitingPlayer)

    case SquadInvitationManager.PerformJoinSquad(player, features, position) =>
      JoinSquad(player, features, position)

    case SquadService2.UpdateSquadList(features, changes) =>
      UpdateSquadList(features, changes)

    case SquadService2.UpdateSquadListWhenListed(features, changes) =>
  ...
  def SquadService2ReloadSquadDecoration(faction: PlanetSideEmpire.Value, to: Long): Unit = {
      ...
      case Some(invitedPlayer) if invitingPlayer != invitedPlayer =>
        (GetParticipatingSquad(invitingPlayer), GetParticipatingSquad(invitedPlayer)) match {
          ...
          case (Some(invitersFeatures), Some(invitedFeatures))
            ...
            invitations.tell(SquadInvitationManager.CreateVacancyInvite(tplayer, invitedPlayer, invitersFeatures), self)
          ...
          case (Some(invitersFeatures), Some(invitedFeatures))
            ...
            invitations.tell(SquadInvitationManager.CreateIndirectInvite(tplayer, invitedPlayer, invitedFeatures), self)

          case (Some(features), None) =>
            //the classic situation
            invitations.tell(SquadInvitationManager.CreateVacancyInvite(tplayer, invitedPlayer, features), self)

          case (None, Some(features)) =>
            //indirection;  we're trying to invite ourselves to someone else's squad
            invitations.tell(SquadInvitationManager.CreateIndirectInvite(tplayer, invitedPlayer, features), self)

          case (None, None) =>
            //neither the invited player nor the inviting player belong to any squad
            invitations.tell(SquadInvitationManager.CreateSpontaneousInvite(tplayer, invitedPlayer), self)
  ...
  def SquadActionMembershipProximityInvite(zone: Zone, invitingPlayer: Long): Unit = {
    GetLeadingSquad(invitingPlayer, None) match {
      case Some(features) =>
        invitations.tell(SquadInvitationManager.HandleProximityInvite(zone, invitingPlayer, features), self)
      case _ => ;
    }
  }

  def SquadActionMembershipAccept(tplayer: Player, invitedPlayer: Long): Unit = {
    invitations.tell(SquadInvitationManager.HandleAcceptance(tplayer, invitedPlayer), self)
  }
  ...
  def SquadActionMembershipReject(tplayer: Player, rejectingPlayer: Long): Unit = {
    invitations.tell(
      SquadInvitationManager.HandleRejection(
        tplayer,
        rejectingPlayer,
  ...
  def SquadActionMembershipCancel(cancellingPlayer: Long): Unit = {
    ...
    invitations.tell(SquadInvitationManager.HandleCancelling(cancellingPlayer), self)
  }
  ...
  def SquadActionMembershipPromote(
    ...
    invitations.tell(SquadInvitationManager.HandlePromotion(sponsoringPlayer, promotedPlayer), self)
  }
      ...
      case AutoApproveInvitationRequests(_) =>
        GetOrCreateSquadOnlyIfLeader(tplayer) match {
          case out @ Some(features) =>
            invitations.tell(SquadInvitationManager.HandleDefinitionAction(tplayer, action, features), self)
            out
          case None =>
            None
        }
  ...
  def SquadActionDefinitionSearchForSquadsWithParticularRole(
    ...
    searchData.get(charId) match {
      ...
      case None =>
        val data = SquadService2.SearchCriteria(tplayer.Faction, criteria)
  ...
  private def SquadActionDefinitionSearchForSquadsUsingCriteria(
                                                                 charId: Long,
                                                                 criteria: SquadService2.SearchCriteria
                                                               ): Unit = {
    subs.Publish(
      charId,
  ...
  private def SearchForSquadsResults(criteria: SquadService2.SearchCriteria): List[PlanetSideGUID] = {
  ...
  def JoinSquad(player: Player, features: SquadFeatures, position: Int): Boolean = {
    ...
    subs.UserEvents.get(charId) match {
      case Some(events)
        ...
        invitations.tell(SquadInvitationManager.HandleCleanup(charId), self)
  ...
  def CloseSquad(squad: Squad): Unit = {
    ...
    invitations.tell(SquadInvitationManager.HandleClosingSquad(features), self)
    updateMembers
      .foreach {
  ...
  def LeaveService(charId: Long, sender: ActorRef): Unit = {
    subs.MonitorSquadDetails.subtractOne(charId)
    invitations.tell(SquadInvitationManager.HandleLeave(charId), self)
  ...
  /**
    ...
    * @see `SquadService2.SquadList.Publish`
    * @param features the squad
    * @param changes the optional highlighted aspects of the squad;
    *                these "changes" do not have to reflect the actual squad but are related to the contents of the message
    */
  ...
  def PublishedLists(squads: Iterable[SquadFeatures]): Vector[SquadInfo] = {
    squads.map { features => SquadService2.PublishFullListing(features.Squad) }.toVector
  }
